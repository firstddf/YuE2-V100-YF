...
历史表有 **「种子」与「步数」两列**(`request.json` 里没有则显示 `-`);
选中任务后还会在下方摊开**该次生成的全部参数**,以及**引擎实际收到的风格与歌词原文**
(收在「风格与歌词原文」折叠面板里 —— 它们是原样下发的长文本,不折叠会把页面撑得很长)。

参数表由 `param_markdown()` 渲染,它**按 `request` 里实际存在的键**出表,而不是列一张固定表 ——
因为服务端会丢掉"界面没填"的项(数值 0 视为未设置),不同任务的键本来就不一样。
实测 23 个有 `request.json` 的历史任务,全部键都被渲染出来,无遗漏。

> 为什么值得专门做这个:`request.json` 里存的是**引擎真正收到的字符串**。
> 比如把整段 Markdown 说明粘进风格框、把 `(音乐:…)` 这类舞台指示留在歌词里,
> 在这里一眼就能看见 —— 它们在界面上看输入框时并不明显。
